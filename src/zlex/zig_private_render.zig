// borrow this file from zig's source https://github.com/ziglang/zig/blob/master/lib/std/zig/render.zig
// for printing and understand zig Ast
...
fn renderMember(
    ...
    const tree_tags = tree.nodes.items(.tag);
    switch (tree_tags[decl]) {
        .fn_decl => {
            try ais.writer().print("[{any}:{d}]<", .{ tree_tags[decl], decl });
            defer ais.writer().print(">", .{}) catch {};
            // Some examples:
            // pub extern "foo" fn ...
            // export fn ...
        ...
        .@"comptime" => {
            try ais.writer().print("[{any}:{d}]<", .{ tree_tags[decl], decl });
            defer ais.writer().print(">", .{}) catch {};
            return renderExpression(r, decl, space);
        },
...
fn renderToken(r: *Render, token_index: Ast.TokenIndex, space: Space) Error!void {
    ...
    try ais.writer().print("[T:{d}]", .{token_index});
